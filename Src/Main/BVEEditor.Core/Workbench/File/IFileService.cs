...
namespace BVEEditor.Workbench
{
	...
	public interface IFileService
	{
		...
		ViewDocumentViewModel OpenFile(FileName fileName, bool switchToOpenedView = true);
		...
		/// <param name="switchToOpenedView">Specifies whether to switch to the view for the specified file.</param>
		/// <returns>The existing or opened <see cref="IViewContent"/> for the specified file.</returns>
		//ViewDocumentViewModel OpenFileWith(FileName fileName, IDisplayBinding displayBinding, bool switchToOpenedView = true);
		...
		ViewDocumentViewModel NewFile(string defaultName, string content);
		...
		/// <param name="defaultName">The (unsaved) name of the to open</param>
		/// <param name="content">Content of the file to create</param>
		ViewDocumentViewModel NewFile(string defaultName, byte[] content);
		...
		/// </summary>
		ViewDocumentViewModel GetOpenFile(FileName fileName);
		
		/// <summary>
		/// Opens the specified file and jumps to the specified file position.
		/// Line and column start counting at 1.
		/// </summary>
		ViewDocumentViewModel JumpToFilePosition(FileName fileName, int line, int column);
		#endregion
		...
		event EventHandler<FileEvent> FileCreated;
		event EventHandler<FileCancelEvent> FileReplacing;
		event EventHandler<FileEvent> FileReplaced;
		#endregion*/
	}
}
