...
namespace ICSharpCode.Core
{
	...
	public static class ResourceService
	{
		static IResourceService Service {
			get { return IoC.Get<IResourceService>(); }
		}
		...
		class ResourceAssembly
		{
			...
			ResourceManager TrySatellite(string language)
			{
				...
				if (File.Exists(fileName)) {
					DebugLogger.Instance.Info("Loging resources " + baseResourceName + " loading from satellite " + language);
			...
			public void Load()
			{
				...
				if (assembly.GetManifestResourceInfo(baseResourceName + "." + currentLanguage + ".resources") != null) {
					DebugLogger.Instance.Info(logMessage + " loading from main assembly");
				...
				{
					DebugLogger.Instance.Info(logMessage + " loading from main assembly (no country match)");
				...
				if (manager == null) {
					DebugLogger.Instance.Warn(logMessage + "NOT FOUND");
				} else {
					if (isIcons)
